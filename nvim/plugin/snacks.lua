...
require('snacks').setup {
  ...
  dashboard = {
    ...
    sections = {
      { section = 'header' },
      {
        pane = 2,
        section = 'terminal',
        cmd = '~/bin/shell-color-scripts/colorscripts/square',
        height = 5,
        padding = 1,
      },
  ...
  words = { enabled = true },
  scroll = { enabled = true, animate = { duration = { step = 20, total = 160 } } },
  styles = {
    notification = {
...
-- Keymaps (ported from the lazy `keys` table). Each entry:
-- { lhs, rhs, desc = ..., mode = ..., nowait = ... }
local keys = {
  ...
  { '<leader>pp', function() Snacks.toggle.profiler() end, desc = 'Profiler Toggle' },
  { '<leader>ph', function() Snacks.toggle.profiler_highlights() end, desc = 'Profiler Highlights' },
...
for _, k in ipairs(keys) do
  vim.keymap.set(k.mode or 'n', k[1], k[2], { desc = k.desc, nowait = k.nowait })
end

-- Globals + toggles (previously gated behind the VeryLazy event; runs eagerly now).
_G.dd = function(...)
